...
//UI.Add
UI.AddLabel("O>>>>>>TAIJIlogist.JS<<<<<<O");
UI.AddLabel("        M i x o l o g i s t # 6105");
UI.AddLabel("         QQgroup:323883471");
UI.AddSliderInt("O====TAIJIlogist.Visual====O", 1, 1);
...
UI.AddCheckbox('TAIJIlogist Damage Numbers');
...
UI.AddSliderFloat("TAIJIlogist kill effect", 0, 2);
...
UI.AddSliderInt("O====TAIJIlogist.Rage/AA====O", 1, 1);
...
UI.AddSliderInt("O====TAIJIlogist.OFFSET====O", 1, 1);
...
UI.AddSliderInt("O====TAIJIlogist.Fakelag====O", 1, 1);
...
function custscope()
{
    ...
    UI.SetEnabled("Script items", "Width", scope_enb);///u s e r:90
...
function resetaa()
{
    if(UI.GetValue("Script items","Reset AA override"))
    {
        ...
        Cheat.PrintChat("[TAIJI] Reset AA override = 1 ");
        Cheat.PrintChat("[TAIJIのOTC] 每局新游戏开始 || AA && FL失效，请重新加载JS");
        Cheat.PrintChat("[MIXO] QQgroup:323883471");
        UI.SetValue("Script items","Reset AA override",0);
    }
}
...
{
  func.apply(null, safe_concat([name], props));///u s e r:90
  return name;
}
...
function set(path, value)///u s e r:90
{
...
function watermark() {
    ...
    var font = Render.AddFont("comic sans ms", 11, 800);///u s e r:90
...
function HUD_REDRAW()
{
    if(loadFont == 0)
    {
        ...
        loadFont = 1;///u s e r:90
    }
    if(!HitAttack || !UI.GetValue("Script items", "TAIJIlogist Damage Numbers"))    return;
...
function bullet() {
    ...
    for (i in traceContainer) {
        ...
        if(traceContainer[i].col[3] <= 15){
            traceContainer.shift()///u s e r:90
        }
...
function AAAIND() {
    ...
    Render.StringCustom(x / 2 +3, y / 2 + 40, 1,"TAIJI",[255, 255, 255, 255], yawfont)
    Render.StringCustom(x / 2 +4, y / 2 + 41, 1,"TAIJI",[255, 255, 255, 255], yawfont)
}
//绘制
function DMGover()
{
...
//[TAIJIlogist CFG] 慢走速度(很多大蛇喜欢叫假走)
function cMove() 
{
...
function OnHurt()
{
    ...
    if (hitbox == 1 || hitbox == 6 || hitbox == 7)  
    {
        ...
        if (Math.abs(lastHitTime - curtime) > 0.5)  
        {
            lastHitTime = curtime;
            Flip();
            Cheat.PrintChat("[TAIJI] On Hit Anti Brute\n")
        }
    }
}
    ...
    if (Entity.IsValid(entity) && Entity.IsEnemy(entity))
    {
        ...
        if (bodyDist < 85.0)
        {
            ...
            if (VectorDistance(closestRayPoint, fakePos) < VectorDistance(closestRayPoint, realPos))
            {
                ...
                Cheat.PrintChat("[TAIJI] Brute distance "+headDist+"\n")
            }
        }
...
function onVoteOptions() {
    options[0] = Event.GetString("option1")///u s e r:90
...
function onVoteCast() {
    ...
    if (entid) {
        ...
        Global.PrintColor([ 217, 217, 217, 255 ], "[TAIJI] \0");
...
function skeetbar() {
    ...
    var fonts = Render.AddFont("Verdana", 10, 800);///u s e r:90
...
function getHitboxName(index)
{
    switch (index)
    {
        ...
        case 8:
            ...
            break;
        case 9:
            hitboxName = "left leg";///u s e r:90
            break;
        case 10:
            hitboxName = "right leg";
            break;
        ...
        case 14:
            ...
            break;
        case 15:
            hitboxName = "left arm";///u s e r:90
            break;
        case 16:
            hitboxName = "left arm";
            break;
...
///u s e r:90
    ...
    if (weapon == "hegrenade")
      ...
      hittype = "Knifed ";
    if (local_player == attackerIndex && local_player != victimIndex) {///u s e r:90
        Cheat.PrintColor([159, 202, 43, 255], "[TAIJI]");
    if (hittype == "Hurt ") {
...
function hitlogss() {///u s e r:90
    ...
    if (shots_fired > hits && (Globals.Curtime() - lastUpdate > 0.33)) {
      ...
      if (!logged) {
        logged = true;
        var issafe = "true";
        var reason = "Resolver";///u s e r:90
        if (safety == 0) {
          issafe = "false";
        }
        ...
        Cheat.PrintColor([159, 102, 243, 255], "[TAIJI] ");
...
function render_effect()
{
    ...
    inc_alpha = ((1 / get("TAIJIlogist kill effect")) * Global.Frametime()) * 255
    inc_size = ((1 / get("TAIJIlogist kill effect")) * Global.Frametime()) * 360
...
Global.RegisterCallback("Draw", "render_effect");///u s e r:90
...
Global.RegisterCallback("Draw", "onshotfl");///u s e r:90
...
Cheat.RegisterCallback("CreateMove","resetaa");///u s e r:90
```
